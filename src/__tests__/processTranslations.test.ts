...
describe('processTranslations', () => {
  ...
  it('should return success=0 and failed=0 when given empty array', async () => {
    ...
    const result = await processTranslations(untranslatedKeys);
  ...
  it('should translate a single key at root level', async () => {
    ...
    expect(translateKey).toHaveBeenCalledWith('login', 'fr', undefined);
  ...
  it('should translate multiple keys', async () => {
    ...
    expect(translateKey).toHaveBeenNthCalledWith(1, 'login', 'fr', undefined);
    expect(translateKey).toHaveBeenNthCalledWith(2, 'logout', 'fr', undefined);
    expect(translateKey).toHaveBeenNthCalledWith(3, 'home', 'fr', undefined);
  });
  ...
  it('should translate a nested key', async () => {
    ...
    expect(result).toEqual({ success: 1, failed: 0 });
    expect(translateKey).toHaveBeenCalledWith('title', 'fr', undefined);
  ...
  it('should translate keys in multiple locales', async () => {
    ...
    expect(result).toEqual({ success: 3, failed: 0 });
    expect(translateKey).toHaveBeenNthCalledWith(1, 'login', 'fr', undefined);
    expect(translateKey).toHaveBeenNthCalledWith(2, 'login', 'es', undefined);
    expect(translateKey).toHaveBeenNthCalledWith(3, 'login', 'de', undefined);
  });
  ...
  it('should use reference context when provided', async () => {
    ...
    const result = await processTranslations(untranslatedKeys, referenceContext);

    expect(result).toEqual({ success: 1, failed: 0 });
    expect(translateKey).toHaveBeenCalledWith('login', 'fr', referenceContext);
  });
  ...
  it('should use bundle-specific reference when available', async () => {
    ...
    const result = await processTranslations(untranslatedKeys, undefined, bundleReferenceFolder);
    ...
    expect(translateKey).toHaveBeenCalledWith('login', 'fr', 'Admin panel context');
  });

  // Test 10 : combine global and bundle-specific contexts
  it('should combine global and bundle-specific contexts', async () => {
    const untranslatedKeys: UntranslatedKey[] = [
      {
        keyPath: ['login'],
        locale: 'fr',
    ...
    // Verify that the two contexts are combined with a line break
    expect(translateKey).toHaveBeenCalledWith(
      'login',
      'fr',
      'Global reference context\n\nAdmin bundle specific context'
    );
  });
});
