...
describe('run', () => {
  beforeEach(() => {
    ...
    (initializeOpenAI as jest.Mock).mockReset();
  ...
  it('should return success when no untranslated keys found', async () => {
    ...
    expect(initializeOpenAI).toHaveBeenCalledWith('test-api-key');
  ...
  it('should translate untranslated keys successfully', async () => {
    ...
    expect(initializeOpenAI).toHaveBeenCalledWith('test-api-key');
    expect(processTranslations).toHaveBeenCalledWith(mockUntranslatedKeys, undefined, undefined);
  ...
  it('should match snapshot for complete workflow', async () => {
    ...
    const workflow = {
      result,
      initializeOpenAICalled: (initializeOpenAI as jest.Mock).mock.calls[0],
